...
* 코드 영역: 스레드 간에 공유됨 -> 어떤 함수든지 모두 스레드에 적재햐여 실행 가능
    ...
    * read-only이기 때문에 프로그램 실행 중에는 어떤 스레드도 코드 영역 내용 변경 불가능 -> thread safety issue 발생 x 
...
* 스택 영역: 
    * 스레드 전용 공간이지만 실제 구현 측면에서는 엄밀히 격리된 스레드 전용 공간은 x 
        ...
        * 버그 발생시 오류 원인 파악이 어려움 -> (내 생각: 머신러닝을 활용하면...?)
```c++
void foo(int* p){
    *p=2; 
}
...
```
* 새로 생성된 스레드가(t) 주 스레드 전용인 데이터를 수정하는 결과를 얻음 
    * -> 즉 스택 영역은 스레드 전용 데이터에 속하지만, 스택 영역에는 별도의 보호를 위한 작동 방식 x 
...
* 스레드 전용 저장소(thread local storage)
    ...
    * 옮기기 귀찮아서 안옮김. p104쪽 예시 코드 참고할 것 