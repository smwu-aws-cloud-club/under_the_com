...
## 02_1_4 
* 프로세스의 단점: entry function이 main 함수밖에 없어 -> 프로세스의 기계 명령어를 한 번에 하나의 CPU에서만 실행 가능 => CPU 여러 개가 동일한 프로세스의 기계 명령어를 실행하게 하도록 프로세스를 여러개의 멀티 스레드로 나누자 
* 스레드: 자신이 속해있는 프로세스의 주소공간을 공유하여, 다중 프로세스 프로그래밍과 달리 훨씬 가볍고, 생성속도가 빠름(프로세스간 통신이 일어나지 않음) 
    ...
    * 다중 코어를 사용하여 CPU를 최대한 활용 가능해짐 (그렇다고 해서 무조건 다중 코어일 필욘 없음! 단일 코어도 됨)
        * 스레드는 운영 체제 계층에서 구현되며 코어 개수와는 무관하기 때문 
    * 단점: CPU가 명령어를 실행할 때 스레드를 고려하지 않기 때문에 다중 스레드가 공유 리소스에 접근 시 오류 발생 가능 =>  mutual exclusion, synchronization을 통해 명시적으로 직접 해결 필요 


## 02_1_5
* 스택 프레임에 parameter, 지역 변수, return address를 저장 
* 모든 함수는 실행 시에 자신만의 runtime stack frame 가짐 
* 함수가 호출/반환 시마다 LIFO 순으로 증가 감소함 -> 이 증감이 프로세스 주소 공간에서 스택 영역을 형성함 
* 프로세스 주소 공간에 각 스레드를 위한 스택 영역이 별도로 있어야함 => 멀티 스레드는 프로세스의 메모리 공간이 소모된다 
    * 필요한 이유: 실행 흐름 여러개(스레드)를 가진 프로세스는 각 흐름(스레드)가 실행될 때 정보를 저장해야하므로 
