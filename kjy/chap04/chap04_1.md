...
## 04_1~3
* 트랜지스터: 한쪾에 전류를 흘리면 나머지 단자 두 개에 전류를 흐르게 하거나 흐르지 못하게 하는 것(Like 스위치)
* 논리곱, 논리합, 논리부정 
* `논리적 완전성(logical completeness)`: 충분한 논리곱, 논리합, 논리부정 게이트가 있으면 어떠한 논리 함수도 구현 가능 (이외에는 필요 x)
...
* `부정 논리곱 게이트(non-conjunction gate, NAND)` : 논리곱 연산 처리후, 논리 부정 연산을 처리함 
    ...
    * 2가지 상태만 존재 가능해짐 => 단자 a값을 회로의 출력으로 사용 (p286 그림4-4 참고)
        * S, R 단자에 1이 입력된다면, 아래와 같은 2가지 상태만 가질 수 있음 
            1. a=1 -> B=0, A=1, b=0 
            2. a=0 -> B=1, A=0, b=1 
            * => 단자 a 값을 회로의 출력으로 사용 
    * 회로에 정보를 저장할 수 있음 (1비트)
        * S=0, (R=1)이면, 회로의 출력값인 단자 a는 항상 1 => 회로에 1이 저장됨
        * R=0, (S=1)이면, 회로의 출력값인 단자 a는 항상 0 => 회로에 0이 저장됨 
* 지금은 정보 저장 위해 S, R 동시에 설정하나, 실제로 저장하는데 필요한 입력은 비트 1개
    * WE(Write Enable) 단자는 저장 여부 선택하는데 사용됨 
    * p287 그림 4-5 참고 
    * D단자가 0이면 전체 회로가 저장하는 것은 0, 그렇지 않으면 1 
...
* `instruction set(명령어 집합)`: 실행할 수 있는 `명령어(opcode)`, 각 명령어에 필요한 `피연산자(operand)`를 묶은 것 
    ...
    * 예시: 16비트의 기계 명령어 (p291 그림 4-8 참고)
...
* 예를 들어 가산기가 작동 시, 레지스터 R1, R2에 반드시 1을 저장하도록 어떻게 보장? -> 각 부분의 회로가 함께 작업할 수 있도록 조정 or 동기화하려면? 
    * `클럭 신호(clock signal)`: 클럭 신호가 전압을 변경할때마다 전체 회로의 각 레지스터, 즉 전체 회로 상태가 갱신됨 => 전체 회로가 함께 동작하도록 할 수 있음 
